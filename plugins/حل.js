...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        if (!m.quoted || !/image/.test(m.quoted.mimetype)) {
            return m.reply(
                `*『 ⚠️ 』يـرجـى الـرد عـلـى صـورة لإتـمـام الـعـمـلـيـة.*\n\n*📌 مـثـال:*\n${usedPrefix + command} مـاذا يـوجـد هـنـا؟`
            )
        }
        ...
        if (!fullText) return m.reply("❌ عـفـواً، لـم يـتـم الـرد مـن الـنـظـام.")

        let finalMsg = `
*╭───〔 🔱 نـتـيـجـة الـتـحـلـيـل 〕───╼*
*│*
${fullText}
*│*
*╰────────────────────╼* 🐗`.trim()
        ...
        m.reply("❌ حـدث خـطأ أثـنـاء مـعـالـجـة الـصـورة.")
    }
}

handler.help = ['حل <رد على صورة>']
